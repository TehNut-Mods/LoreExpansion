...
public class PlayerEventHandler {
    ...
    public void playerTick(TickEvent.PlayerTickEvent event) {
        ...
        for (int i = 0; i < inventoryPlayer.getSizeInventory(); i++) {
            if (inventoryPlayer.getStackInSlot(i) != null && inventoryPlayer.getStackInSlot(i).getItem() == LoreExpansion.LORE_PAGE) {
                ...
                if (lore.shouldAutoAdd() && !LoreUtil.getData(event.player.getEntityWorld()).getDataForPlayer(event.player).contains(lore.getKey())) {
    ...
    public void onItemPickup(EntityItemPickupEvent event) {
        ...
        if (stack.getItem() == LoreExpansion.LORE_PAGE && stack.hasTagCompound()) {
            ...
            if (lore.shouldAutoAdd() && !LoreUtil.getData(event.getEntityPlayer().getEntityWorld()).getDataForPlayer(event.getEntityPlayer()).contains(lore.getKey())) {
